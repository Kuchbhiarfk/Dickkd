...
ENCRYPTION_KEY = bytes.fromhex('0123456789abcdef0123456789abcdef')  # 16 bytes (32 hex chars)
IV = b'abcdef9876543210'  # 16 bytes (raw bytes, not hex-decoded)
...
def encrypt_json(data: dict) -> str:
    """
    Encrypt JSON data using AES-256-CBC to match JavaScript encryption.
...
async def download_and_encrypt_json(client: Client, msg: Message, user_first_name: str, user_id: str, made_at: str) -> Tuple[str, str]:
    """
    Download a .json file from a message, add user details, and encrypt it.
    Returns the encrypted data (IV:encrypted, Base64-encoded) and the original filename.
    """
    ...
    try:
        ...
        # Encrypt the modified JSON data
...
async def upload_encrypted_json(client: Client, encrypted_data: str, filename: str, chat_id: int) -> Message:
    """
    Upload encrypted JSON data as a .json file to the specified chat.
        ...
        if link_type == "HACKHEIST":
            ...
            for msg in messages:
                ...
                if msg.document and msg.document.file_name.endswith('.json'):
                    try:
                        encrypted_data, filename = await download_and_encrypt_json(client, msg, user_first_name, user_id, made_at)
                        encrypted_msg = await upload_encrypted_json(client, encrypted_data, filename, message.from_user.id)
