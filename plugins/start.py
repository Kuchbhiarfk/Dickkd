...
# Encryption configuration - SAME as JavaScript
ENCRYPTION_KEY = b'0123456789abcdef0123456789abcdef'  # 32 bytes
IV = b'abcdef9876543210'  # 16 bytes

def encrypt_data(text):
    """Encrypt text using AES-256-CBC - Compatible with JavaScript"""
    ...
    # Return in same format as JavaScript: IV:EncryptedData (both base64)
...
def decrypt_data(encrypted_text):
    """Decrypt text - Compatible with JavaScript"""
    try:
...
# Dictionary to store user data when they use /start
user_start_data = {}
    ...
    try:
        ...
        if msg.video or msg.document or msg.photo or msg.audio or msg.animation or msg.sticker:
            try:
                ...
                # Fallback: Forward to dump channel, copy, download
                try:
...
# NEW COMMAND: Upload JSON file to encrypt
@Bot.on_message(filters.command('upload_json') & filters.private & filters.user(ADMINS))
async def upload_json_handler(client: Bot, message: Message):
    if not message.reply_to_message or not message.reply_to_message.document:
        await message.reply_text("❌ Please reply to a JSON file with /upload_json command.")
        return
    
    doc = message.reply_to_message.document
    if not doc.file_name.endswith('.json'):
        await message.reply_text("❌ Please upload a valid .json file.")
        return
    
    status_msg = await message.reply_text("⏳ Processing JSON file...")
    ...
    try:
        ...
        if not isinstance(json_data, list):
            await status_msg.edit("❌ JSON file must contain an array of objects.")
            os.remove(file_path)
            return
        
        # Get user data from user_start_data
        user_id_str = str(message.from_user.id)
        user_info = user_start_data.get(user_id_str, {
            'user_first_name': message.from_user.first_name,
            'user_id': user_id_str,
            'made_at': time.strftime('%Y-%m-%dT%H:%M:%S')
        })
        
        # Process each item and encrypt required fields
        encrypted_data = []
        for item in json_data:
            encrypted_item = {
                'class_name': encrypt_data(item.get('class_name', '')),
                'teacher_name': encrypt_data(item.get('teacher_name', '')),
                'thumbnail': encrypt_data(item.get('thumbnail', '')),
                'class_url': encrypt_data(item.get('class_url', '')),
                'slides_url': encrypt_data(item.get('slides_url', '')),
                'is_offline': encrypt_data(str(item.get('is_offline', 'false'))),
                'live_at_time': encrypt_data(item.get('live_at_time', item.get('live_at', ''))),
                'user_first_name': encrypt_data(user_info['user_first_name']),
                'user_id': encrypt_data(user_info['user_id']),
                'made_at': encrypt_data(user_info['made_at'])
            }
            encrypted_data.append(encrypted_item)
        
        # Save encrypted data to new JSON file
        output_filename = f"encrypted_{doc.file_name}"
        ...
        await client.send_document(
            ...
            caption=f"✅ Encrypted JSON file ready!\n\n"
                    f"📊 Total items: {len(encrypted_data)}\n"
                    f"👤 User: {user_info['user_first_name']} (ID: {user_info['user_id']})\n"
                    f"⏰ Created at: {user_info['made_at']}\n\n"
                    f"🔐 All fields are encrypted with AES-256-CBC"
        )
        ...
        await status_msg.edit("❌ Invalid JSON format in the file.")
...
# NEW COMMAND: Test decryption
@Bot.on_message(filters.command('test_decrypt') & filters.private & filters.user(ADMINS))
async def test_decrypt_handler(client: Bot, message: Message):
    if not message.reply_to_message or not message.reply_to_message.document:
        await message.reply_text("❌ Please reply to an encrypted JSON file with /test_decrypt command.")
        return
    
    status_msg = await message.reply_text("⏳ Testing decryption...")
    ...
    try:
        ...
        if not isinstance(encrypted_data, list) or len(encrypted_data) == 0:
            await status_msg.edit("❌ Invalid encrypted JSON format.")
            os.remove(file_path)
            return
        
        # Test decrypt first item
        first_item = encrypted_data[0]
        decrypted_sample = {}
        
        for key, encrypted_value in first_item.items():
            try:
                decrypted_sample[key] = decrypt_data(encrypted_value)
            except Exception as e:
                decrypted_sample[key] = f"[Decryption failed: {str(e)}]"
        
        # Format output
        output = "✅ Decryption Test Result (First Item):\n\n"
        output += json.dumps(decrypted_sample, indent=2, ensure_ascii=False)
        
        await status_msg.edit(f"```json\n{output}\n```", parse_mode=ParseMode.MARKDOWN)
