...
async def anonymize_message(client: Client, original_msg: Message):
    """
    Anonymize a non-anonymous message by forwarding to dump channel, copying it, deleting the forwarded, and returning the copied message.
    """
    try:
        forwarded = await original_msg.forward(DUMP_CHANNEL_ID)
        copied = await forwarded.copy(DUMP_CHANNEL_ID)
        await forwarded.delete()
        return copied
    except Exception as e:
        print(f"Failed to anonymize message {original_msg.id}: {e}")
        return original_msg  # Fallback to original if anonymization fails

def is_anonymous(msg: Message):
    """
    Check if the message is purely anonymous (no user, no forward info).
    """
    return (
        msg.from_user is None
        and msg.forward_from is None
        and msg.forward_from_chat is None
        and not msg.forward_sender_name
        and not msg.forward_signature
    )
...
@Bot.on_message(filters.group & filters.media & ~filters.forwarded & subscribed)
async def handle_group_non_anonymous_media(client: Client, message: Message):
    """
    Handle non-anonymous media messages in groups by forwarding to dump channel,
    downloading, and uploading back to the group.
    """
    try:
        ...
        if not chat_member.privileges:
            await message.reply("❌ Bot must be an admin in the group to process messages.")
            return

        # Forward the message to the dump channel with forward tag
        forwarded_msg = await message.forward(DUMP_CHANNEL_ID)
        if not forwarded_msg:
            await message.reply("❌ Failed to forward message to dump channel.")
            return

        # Fetch the forwarded message from the dump channel
        try:
            dump_msg = await client.get_messages(DUMP_CHANNEL_ID, forwarded_msg.id)
            if not dump_msg:
                await message.reply("❌ Failed to fetch forwarded message from dump channel.")
                return
        except (ChannelInvalid, PeerIdInvalid, BadRequest, Exception) as e:
            await message.reply(f"❌ Error fetching message from dump channel: {str(e)}")
            return

        # Prepare message details
        filename = "Unknown"
        media_type = "Unknown"
        caption = dump_msg.caption.html if dump_msg.caption else "🔥 𝐇𝐈𝐃𝐃𝐄𝐍𝐒 🔥"

        if dump_msg.video:
            media_type = "Video"
            filename = dump_msg.video.file_name if dump_msg.video.file_name else "Unnamed Video"
        elif dump_msg.document:
            filename = dump_msg.document.file_name if dump_msg.document.file_name else "Unnamed Document"
            ...
            media_type = "Audio"
            filename = dump_msg.audio.file_name if dump_msg.audio.file_name else "Unnamed Audio"
        elif dump_msg.animation:
            media_type = "Animation"
            filename = dump_msg.animation.file_name if dump_msg.animation.file_name else "Unnamed Animation"
        elif dump_msg.sticker:
            media_type = "Sticker"
            filename = "Sticker"
        ...
        # Download media
        path = None
        if dump_msg.video or dump_msg.document or dump_msg.photo or dump_msg.audio or dump_msg.animation or dump_msg.sticker:
            try:
                path = await client.download_media(dump_msg)
            except Exception as e:
                print(f"Failed to download media: {e}")
                # Try sending with file_id if download fails
        ...
        try:
            if dump_msg.sticker:
                copied_msg = await client.send_sticker(
                    chat_id=message.chat.id,
                    sticker=path if path else dump_msg.sticker.file_id,
                    protect_content=PROTECT_CONTENT
                )
            elif dump_msg.photo:
                copied_msg = await client.send_photo(
                    chat_id=message.chat.id,
                    photo=path if path else dump_msg.photo.file_id,
                    caption=caption,
                    parse_mode=ParseMode.HTML,
                    protect_content=PROTECT_CONTENT
                )
                ...
                copied_msg = await client.send_video(
                    chat_id=message.chat.id,
                    video=path if path else dump_msg.video.file_id,
                    caption=caption,
                    parse_mode=ParseMode.HTML,
                    protect_content=PROTECT_CONTENT
                )
                ...
                copied_msg = await client.send_document(
                    chat_id=message.chat.id,
                    document=path if path else dump_msg.document.file_id,
                    caption=caption,
                    parse_mode=ParseMode.HTML,
                    protect_content=PROTECT_CONTENT
                )
                ...
                copied_msg = await client.send_audio(
                    chat_id=message.chat.id,
                    audio=path if path else dump_msg.audio.file_id,
                    caption=caption,
                    parse_mode=ParseMode.HTML,
                    protect_content=PROTECT_CONTENT
                )
                ...
                copied_msg = await client.send_animation(
                    chat_id=message.chat.id,
                    animation=path if path else dump_msg.animation.file_id,
                    caption=caption,
                    parse_mode=ParseMode.HTML,
                    protect_content=PROTECT_CONTENT
                )
            ...
            print(f"Failed to upload media to group: {e}")
            await message.reply("❌ Failed to upload media to the group.")
        finally:
            if path:
                try:
                    os.remove(path)
        ...
        special_msg = await send_random_special_message(client, message.chat.id)
        if special_msg:
        ...
        k = await client.send_message(
            chat_id=message.chat.id,
            text=f"<b>🔥 Hurry! This Lecture/PDF will be <u>deleted automatically in 4 hours</u> ⏳</b>\n\n"
                 f"<b>⚡ Save it - Forward, Download & Keep in your Gallery before time runs out!</b>\n\n"
        ...
        await message.reply("❌ Bot requires admin privileges in the group and dump channel.")
    except Exception as e:
    ...
    if len(text) > 7:
        ...
        if link_type == "HACKHEIST":
            ...
            for original_msg in messages:
                ...
                # Anonymize if not anonymous
                if not is_anonymous(original_msg):
                    msg = await anonymize_message(client, original_msg)
                    is_anonymized = True
                else:
                    msg = original_msg
                    is_anonymized = False
                ...
                if msg.video or msg.document or msg.photo or msg.audio or msg.animation or msg.sticker:
                    try:
                        ...
                        print(f"Failed to download media: {e}")
                        # Removed continue; try sending with file_id instead
                ...
                try:
                    ...
                    # Delete anonymized msg if applicable
                    if is_anonymized:
                        await msg.delete()
